	...
	function __construct()
  {
	...
	public function save_register(){
		if($post = $this->input->post('form')){
			...
			$this->form_validation->set_rules('form[Address]', 'Address', 'required');
			$this->form_validation->set_rules('form[Age]', 'Age', 'required');
			$this->form_validation->set_rules('form[Email]', 'Email', 'required|valid_email');
			$this->form_validation->set_rules('form[ContactNo]', 'Contact No', 'required');
	...
	public function check_login(){
		if($this->input->post('Username')){
			...
			if($this->form_validation->run()){
				 ...
				 $user = $this->model->login($this->input->post('Username'), sha1($this->input->post('Password')));
				 // print_r($user);
				 if($user){
				 	...
				 	$this->session->set_flashdata('notification', '<div class="alert alert-info">
                    <strong>Ops!</strong> check your username or password!!
                  </div>');
